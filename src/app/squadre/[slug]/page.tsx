  ...
  return (
    <div className="min-h-screen p-6 font-sans sm:p-10">
      ...
      <header className="mt-4 mb-6 flex items-center gap-3">
        <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{team.teamName}</h1>
        {team.formation && (
                ...
                {team.byRole[role].map((p) => {
                  ...
                  return (
                    <tr key={p.playerId} className="border-t border-zinc-100 dark:border-zinc-800">
                      ...
                      <td className="px-3 py-2">{p.advice ? `${p.advice.score}/100` : "—"}</td>
                      <td className="px-3 py-2">{p.advice ? bandLabel(p.advice.band) : "—"}</td>
                      <td className="px-3 py-2">
                        {p.ownedByName ? (
                          <span className={p.ownedByIsMe ? "text-emerald-600 dark:text-emerald-400" : ""}>
                            {p.ownedByName} ({p.pricePaid})
                          </span>
                          ...
                          <span className="text-zinc-400">Disponibile</span>
                        )}
                      </td>
                    </tr>
                  );
